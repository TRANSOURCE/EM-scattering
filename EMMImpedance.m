function [EM,NormEM,errorE,d] = EMMImpedance(a,M,vis)
...
fprintf('\nSOLVING ELECTROMAGNETIC SCATTERING PROBLEM BY %d SMALL IMPEDANCE PARTICLES:\n',M);
    ...
    function A = SetupMatrix(MMatrixSizeM)
        ...
        for j=1:M    
            ...
            for s=1:M
                ...
                col = (s-1)*3+1;
                %First component x:                
                A(row,col)=C*(k2*G + PGG1(1));%For x
                A(row,col+1)=C*PGG2(1);%For y
                A(row,col+2)=C*PGG3(1);%For z
                
                %Second component y:                
                A(row+1,col)=C*PGG1(2);
                A(row+1,col+1)=C*(k2*G + PGG2(2));
                A(row+1,col+2)=C*PGG3(2);
                
                %Third component z:                
                A(row+2,col)=C*PGG1(3);
                A(row+2,col+1)=C*PGG2(3);
                A(row+2,col+2)=C*(k2*G + PGG3(3));
            end            
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function RHS = SetupRHS(MMatrixSize)
        ...
        for s=1:M 
            ...
            RHS(row) = e*(ES(3)*alpha(2)-ES(2)*alpha(3));
            RHS(row+1) = e*(ES(3)*alpha(1)-ES(1)*alpha(3));
            RHS(row+2) = e*(ES(2)*alpha(1)-ES(1)*alpha(2));
        end
    end
