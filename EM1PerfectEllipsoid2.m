function [Ea,Ee,J,Ediff,Qdiff] = EM1PerfectEllipsoid(a,b,c,m,w,X,vis)
...
NsVec = NSVec();
Pos = PointPosOnBody();
Delta = ((((a*b)^1.6+(a*c)^1.6+(c*b)^1.6)/3)^(1/1.6))*PI4/M;
...
fprintf('\nRelative error of solving the LAS for J: %E\n',err);
...
% GQ = (Gamma*(Qe.')).'
% GammaQ = FindGammaQ(J)
% ValidateQ = norm(Qe+GammaQ-RHSQ)/norm(RHSQ);
% fprintf('\nValidate Q using GQ, relative error: %E',ValidateQ);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if(vis)
    ...
    Y = Pos*1.5;
    for ii=1:M
        EM3 = [EM3;Easym(ES,alpha,Y(ii,:),Qe)];
    end
    Visualize(EM3,Y);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function GNS = GNS(PointI)
        theta = P2TP(PointI,1);
        phi = P2TP(PointI,2);
        
        GNS = [a*cos(theta)*cos(2*phi),b*sin(theta)*cos(2*phi),-c*sin(2*phi)]; %*sin^2(phi)*abc
        GNS = GNS/norm(GNS);
    end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    function GNVec = GNSVec()
        GNVec = [];        
        for i=1:M
           GNVec = [GNVec;GNS(i)]; 
        end
    ...
    function GammaQ = FindGammaQ(J)
        ...
        for i=1:M
            ...
            for j=1:M
                ...
                GG = -rv/(PI4*(r*r*r));             %GG0
                sum1 = sum1 + GG*NsVec(j,1);     
                sum2 = sum2 + GG*NsVec(j,2);
                sum3 = sum3 + GG*NsVec(j,3);
            end                       
            GammaQ = GammaQ + (sum1*J(i,1) + sum2*J(i,2) + sum3*J(i,3));            
        end        
        GammaQ = GammaQ*Delta*Delta;
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function LHSIE = LHSIE(s)       
        ...
        for t=1:M
            ...
            r = norm(rv,2);            
            GG = -rv/(PI4*(r*r*r));
            sum = sum + (GG*(NsVec(s,1)*J(t,1)+NsVec(s,2)*J(t,2)+NsVec(s,3)*J(t,3)) - J(t,:)*(GG(1)*NsVec(s,1)+GG(2)*NsVec(s,2)+GG(3)*NsVec(s,3)))*Delta;
        end               
        LHSIE = J(s,:) + 2*sum; 
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function Xint1 = Xint1(s)
        ...
        for t=1:M                      
            ...
            GG = -rv/(PI4*(r*r*r));
            N = NsVec(s,:)-NsVec(t,:);
            sum = sum + GG*(N(1)*J(t,1)+N(2)*J(t,2)+N(3)*J(t,3))*Delta;
        end               
        Xint1 = sum; 
    end
