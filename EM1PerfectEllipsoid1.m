...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function GammaQ = FindGammaQ(NsVec,J)
        GammaQ = zeros(1,3);        
        for i=1:M
            sum = zeros(1,3);
            for j=1:M
                ...
                sum = sum + GradGreen(Pos(i,:),Pos(j,:))*(NsVec(i,1)*J(j,1)+NsVec(i,2)*J(j,2)+NsVec(i,3)*J(j,3));
            end
            GammaQ = GammaQ + sum;
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function gamma = FindGamma0(Q,GQ)        
        gamma = zeros(3);    
        sumQ = norm(Q,2)^-2;  
        for p=1:3            
            for q=1:3
                gamma(p,q) = Q(q)*GQ(p)*sumQ;
            end            
        end
    end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function gamma = FindGamma1(Q,GQ)                       
        sumQ = norm(Q,2)^-2;     
        cMin = dot(conj(Q),GQ)*sumQ;
        gamma = eye(3)*cMin;
    end

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

    function gamma = FindGamma(Q,RHSQ)                               
        gamma = zeros(3);
        RHSG = RHSQ-Q;
        if(Q(1)~=0)
            gamma(1,1) = RHSG(1)/Q(1);
        end
        if(Q(2)~=0)
            gamma(1,1) = RHSG(2)/Q(2);
        end
        if(Q(3)~=0)
            gamma(1,1) = RHSG(3)/Q(3); 
        end
    end
